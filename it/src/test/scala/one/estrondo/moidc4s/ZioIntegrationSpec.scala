...
import one.estrondo.moidc4s.jwt.zio._
import one.estrondo.moidc4s.zio._
import one.estrondo.moidc4s.zio.http._
import one.estrondo.moidc4s.zio.json._
...
object ZioIntegrationSpec extends ZIOSpecDefault {
  ...
  override def spec = suite("ZIO Integration")(
    ...
    test("Using moidc4s with a OpenId Provider through the Discovery Endpoint (invalid token).") {
      for {
        client            <- ZIO.service[Client]
        scope             <- ZIO.service[Scope]
        wireMockContainer <- ZIO.service[WireMockContainer]
